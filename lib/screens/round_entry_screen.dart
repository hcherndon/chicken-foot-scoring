...
class _RoundEntryScreenState extends ConsumerState<RoundEntryScreen> {
  ...
  Future<void> _save(Game game) async {
    ...
    try {
      final updated = await ref
          ...
          .submitRound(widget.roundIndex, _entries);
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    final startingDouble =
        game.rules.set.startingDoubleFor(widget.roundIndex);
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Round ${widget.roundIndex + 1}'),
        actions: [
          Padding(
            ...
            child: Row(
              children: [
                ...
                DominoTile(value: startingDouble, height: 36),
              ],
            ),
          ),
        ],
